...
catalog as (select brand_url,
            ...
            where is_404 = false and out_of_stock = false 
              and api_only = false and brand_url <> '' and category_url <> '' and gender <> 0),

catalog_rollup as (select * from (
                    select brand_url, cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id,
                            count(*) as offers,
                            ...
                            row_number() over (partition by brand_url, cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id order by count(*) desc) as rn
                    from catalog
                    group by GROUPING SETS (  (brand_url,cat_l1,gender,store_merchant_id),
                                              (brand_url,cat_l1,cat_l2,gender,store_merchant_id),
                                              (brand_url,cat_l1,cat_l2,cat_l3,gender,store_merchant_id),
                                              (brand_url,cat_l1,cat_l2,cat_l3,cat_l4,gender,store_merchant_id))))
