with                   
catalog as (select brand_url, series_url,
            ...
            where is_404 = false and out_of_stock = false 
              and api_only = false and brand_url <> '' and category_url <> '' and series_url <> '' and gender <> 0),

catalog_rollup as (select * from (
                    select brand_url, series_url, cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id, 
                            count(*) as offers,
                            ...
                            row_number() over (partition by brand_url, series_url, cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id order by count(*) desc) as rn
                    from catalog
                    group by GROUPING SETS (  (brand_url,series_url,gender,cat_l1,store_merchant_id),
                                              (brand_url,series_url,gender,cat_l1,cat_l2,store_merchant_id),
                                              (brand_url,series_url,gender,cat_l1,cat_l2,cat_l3,store_merchant_id),
                                              (brand_url,series_url,gender,cat_l1,cat_l2,cat_l3,cat_l4,store_merchant_id))))
