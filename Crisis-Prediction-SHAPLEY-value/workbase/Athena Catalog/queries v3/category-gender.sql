with                   
catalog as (select case when comparable = true then 1 when comparable = false then 0 end as comparable_offer, 
            ...
            where is_404 = false and out_of_stock = false 
              and api_only = false and category_url <> '' and gender <> 0),

catalog_rollup as (select * from (
                    select cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id, 
                            count(*) as offers, 
                            ...
                            row_number() over (partition by cat_l1, cat_l2, cat_l3, cat_l4, gender, store_merchant_id order by count(*) desc) as rn
                    from catalog
                    group by GROUPING SETS (  (cat_l1,gender,store_merchant_id),
                                              (cat_l1,cat_l2,gender,store_merchant_id),
                                              (cat_l1,cat_l2,cat_l3,gender,store_merchant_id),
                                              (cat_l1,cat_l2,cat_l3,cat_l4,gender,store_merchant_id))))
